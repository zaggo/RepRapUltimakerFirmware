...
{
    int steps_per_mm = toolhead_stepper_data(extruder_in_use)->steps_per_mm;
    if(using_mm)
    {
      units.x = X_STEPS_PER_MM;
      units.y = Y_STEPS_PER_MM;
      units.z = Z_STEPS_PER_MM;
      units.e = steps_per_mm;
      units.f = 1.0;
    } else
    {
...
//if(acceleration_enabled == false) //ERIK
...
//set our direction pins as well
  ...
  set_toolhead_stepper_direction(extruder_in_use, e_direction);
...
void cartesian_dda::enable_steppers()
{
  ...
  if(delta_steps.e)
    set_toolhead_stepper_enable(extruder_in_use, 1);
#endif  
}
...
void cartesian_dda::disable_steppers()
{
	...
	digitalWrite(Y_ENABLE_PIN, !ENABLE_ON);
        ...
        set_toolhead_stepper_enable(extruder_in_use, 0);
...
{
  if(extruder_in_use != NULL) step_toolhead_stepper((struct toolhead_stepper_data *)extruder_in_use->motor_data);
}
