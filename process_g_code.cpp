    ...
    while (talkToHost.gotData() && c != '\n') {
        ...
        if (c >= ' ' || c == '\n') {
            ...
            if (c != '\n') {
                ...
                // Start of comment - used by skeinforge (UNTESTED)
                if (c == '(')
                    comment = true;

                // End of comment - used by skeinforge (UNTESTED)
                if (c == ')')
                    comment = false;
...
int parse_string(struct GcodeParser * gc, char instruction[ ], int size) {
    ...
    for (ind = 0; ind < size; ind += (1 + len)) {
        ...
        switch (instruction[ind]) {
                // isn't it faster to increment the var "ind" with one and use that directly?
...
void process_string(char instruction[], int size) {
        ...
        switch (gc.M) {
            ...
            case 0:
                ...
                /*
                                        ...
                                        case 101:
                                                set_toolhead_stepper_direction(extruder_in_use, 1);
                                                set_toolhead_stepper_speed(extruder_in_use, extruder_speed);
                                                break;

                                        //turn extruder on, reverse
                                        case 102:
                                                set_toolhead_stepper_direction(extruder_in_use, 0);
                                                set_toolhead_stepper_speed(extruder_in_use, extruder_speed);
                                                break;
            ...
            case 104:
                if (gc.seen & GCODE_S && extruder_in_use != NULL && extruder_in_use->heater != NULL) {
                    extruder_in_use->heater->target = (float) gc.S;
                }
                break;
            ...
            case 105:
                ...
                if (extruder_in_use == NULL || extruder_in_use->heater == NULL) {
                    ...
                    temperature = extruder_in_use->heater->current;

                talkToHost.setETemp((int)temperature);
                talkToHost.setBTemp((int)heatedBed->current);
                break;

                //turn fan on
            case 106:
                if (extruder_in_use != NULL && extruder_in_use->fan != NULL) {
                    set_solenoid(extruder_in_use->fan, 1);
                }
                break;
            ...
            case 107:
                if (extruder_in_use != NULL && extruder_in_use->fan != NULL) {
                    set_solenoid(extruder_in_use->fan, 0);
                }
                break;
            ...
            case 109:
                if (extruder_in_use != NULL && extruder_in_use->heater != NULL) {
                    extruder_in_use->heater->target = (float) gc.S;
                    waitForTemperature = extruder_in_use->heater;
                }
                break;
            ...
            case 114:
                ...
                // wait for all extruders and the heated bed to reach temperature
            case 116:
                waitForAllTemperatures = 1;
                break;
                ...
                // Open the valve
            case 126:
                if (extruder_in_use != NULL && extruder_in_use->valve != NULL)
                    set_solenoid(extruder_in_use->valve, 1);
                break;

                // Close the valve
            case 127:
                if (extruder_in_use != NULL && extruder_in_use->valve != NULL)
                    set_solenoid(extruder_in_use->valve, 0);
                break;
    ...
    if (gc.seen & GCODE_T) {
        while (!qEmpty()) delay(WAITING_DELAY);
        //delay(2*WAITING_DELAY);
        extruder_in_use = get_toolhead(gc.T);
    }
}
