...
int heater_read_raw_sensor_input(Heater *h)
{
  ...
  if (raw == 1023 && h->target != 0)
  {
    ...
    if (h->target !=0) error_code = HEATER_SENSOR_DISCONNECTED;
  }
  else
  {
...
int heater_read_sensor(Heater *h)
{
  ...
  talkToHost.put("<sensorName>");
  talkToHost.put(h->sensor);
  talkToHost.put("</sensorName>\n");
  talkToHost.put("<length>");
  talkToHost.put(length);
  talkToHost.put("</length>\n");
  switch(h->sensor)
  {
  ...
  case temperature_sensor_type_max6675_thermocouple:
    ...
    {
      temperature = 2000;
      if (h->target !=0) error_code = HEATER_SENSOR_INTERNAL_ERROR;
    }
    else
      temperature = value/4;
  ...
  talkToHost.put("<temperatureArray>");
  ...
  {
    error_code = HEATER_OVERHEATED;
  }

  h->instantaneous = temperature;
  talkToHost.put(h->instantaneous);
...
{
  talkToHost.put("reading sensor\n");
  int error_code = heater_read_sensor(h);
  talkToHost.put("read sensor\n");
...
//    "Temperature (%f degrees C) above thermal cutoff (extruder overheated) or sensor disconnected. Please fix and reset.", h->current);
  ...
  case HEATER_NOT_HEATING_SENSOR:
    sprintf(message,
    "Heater is on but the temperature is not increasing. Check that both the heater and %s are connected. (target: %d)", sensor_name, h->target);
    break;
  default:
